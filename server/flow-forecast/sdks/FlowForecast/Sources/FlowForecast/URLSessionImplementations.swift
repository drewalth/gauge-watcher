...
public typealias FlowForecastAPIChallengeHandler = (URLSession, URLSessionTask, URLAuthenticationChallenge) -> (
...
private var challengeHandlerStore = SynchronizedDictionary<Int, FlowForecastAPIChallengeHandler>()
...
open class URLSessionRequestBuilder<T>: RequestBuilder<T> {
  ...
  override open func execute(
    _ apiResponseQueue: DispatchQueue = FlowForecastAPI.apiResponseQueue,
  ...
  open func buildHeaders() -> [String: String] {
    ...
    for (key, value) in FlowForecastAPI.customHeaders {
  ...
  public var taskDidReceiveChallenge: FlowForecastAPIChallengeHandler?
    ...
    completion: @escaping (_ result: Swift.Result<Response<T>, ErrorResponse>) -> Void)
  {
    ...
    case is URL.Type:
      do {
        ...
        if
          let headerFileName = getFileName(fromContentDisposition: httpResponse
            .allHeaderFields["Content-Disposition"] as? String)
        {
          requestPath = requestPath.appending("/\(headerFileName)")
        } else {
          requestPath = requestPath.appending("/tmp.FlowForecast.\(UUID().uuidString)")
        }
...
private class FormDataEncoding: ParameterEncoding {
  ...
  func mimeType(for url: URL) -> String {
    ...
    if #available(macOS 11.0, iOS 14.0, tvOS 14.0, watchOS 7.0, *) {
      ...
      if
        let uti = UTTypeCreatePreferredIdentifierForTag(kUTTagClassFilenameExtension, pathExtension as NSString, nil)?
          .takeRetainedValue(),
